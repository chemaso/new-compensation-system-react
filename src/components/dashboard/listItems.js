  ...
  return (
    <div>
      {items?.map((item) => {
        ...
        return (
          <>
            <ListItem key={item.title} button onClick={() => onChange(item.title, { section:'title', route: item.route })} style={{ height: 35, background: (isSelected && isEmpty(subSelected)) ? 'linear-gradient(45deg, #f7aa37 30%, #ff600d 90%)' : 'transparent' }}>
              <ListItemIcon>
                <DashboardIcon style={{ color: 'white' }} />
              </ListItemIcon>
              <ListItemText primary={item.title} style={{ color: 'white' }} />
              {!isEmpty(item.subcontent) && <IconButton>
                {isSelected ? <ExpandLess style={{ color: 'white' }} /> : <ExpandMore style={{ color: 'white' }} />}
              </IconButton>}
            </ListItem>
              ...
              item.subcontent.map(val => {
                ...
                <Collapse key={val.title} in={isSelected || isSubSelected} timeout="auto">
                  <List component="div" disablePadding>
                    <ListItem button  style={{ paddingLeft: 30, height: 35, background: isSubSelected ? 'linear-gradient(45deg, #f7aa37 30%, #ff600d 90%)' : 'transparent' }} onClick={() => onChange(val.title, {section: 'subcontent', route: val.route})}>
                      <ListItemIcon>
                        <LayersIcon style={{ color: 'white' }} />
                      </ListItemIcon>
                      <ListItemText primary={val.title} style={{ color: 'white' }} />
                    </ListItem>
                  </List>
                </Collapse>
              )}))}
