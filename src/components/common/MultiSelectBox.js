...
export default function MultiSelectBox({ options = [], onChange, selection }) {
  ...
  const filtered = (values, pos) => values.filter((item) => item.label.toLowerCase().indexOf(filter[pos].toLowerCase().trim()) !== -1)
  return (
    <Grid container className={classes.root}>
      ...
      >
        <div>
          {" "}
          <IconButton aria-label="delete" color="primary" onClick={() => handleSelected('go')}>
            <ArrowForwardIcon  style={{ color: '#EF793C'}} fontSize="large" />
          </IconButton>
        </div>
        <div>
          {" "}
          <IconButton onClick={() => handleSelected('back')} aria-label="delete" color="primary">
